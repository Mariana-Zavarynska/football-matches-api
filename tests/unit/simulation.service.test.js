...
const FakeClock_1 = require("./FakeClock");
class FakePublisher {
    events = [];
...
const getTotalGoals = (sim) => sim.matches.reduce((sum, m) => sum + m.score.home + m.score.away, 0);
describe('SimulationService', () => {
    ...
    let service;
    beforeEach(() => {
        repository = new inMemorySimulation_repository_1.InMemorySimulationRepository();
        publisher = new FakePublisher();
        clock = new FakeClock_1.FakeClock();
        service = new simulation_service_1.SimulationService(repository, publisher, clock);
    });
    it('should start simulation successfully', async () => {
        ...
        expect(publisher.events).toEqual(expect.arrayContaining([
    ...
    it('should not allow starting more often than once per 5 seconds', async () => {
        ...
        await expect(service.start('Qatar 2024')).rejects.toThrow('Simulation cannot be started more frequently than once per 5 seconds');
    });
    ...
    it('should publish goal events every second', async () => {
        await service.start('Katar 2023');
        await clock.advanceBy(3000);
        const goalEvents = publisher.events.filter((e) => e.event === 'score_updated');
    ...
    it('should finish automatically after 9 seconds with total 9 goals', async () => {
        ...
        const finishedEvent = publisher.events.find((e) => e.event === 'simulation_finished');
